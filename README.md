...
```sh
make docker-build docker-push IMG=<some-registry>/hdfs-crd:tag
```
...
```sh
make deploy IMG=<some-registry>/hdfs-crd:tag
```
...
```sh
make build-installer IMG=<some-registry>/hdfs-crd:tag
```
...
```sh
kubectl apply -f https://raw.githubusercontent.com/<org>/hdfs-crd/<tag or branch>/dist/install.yaml
```
